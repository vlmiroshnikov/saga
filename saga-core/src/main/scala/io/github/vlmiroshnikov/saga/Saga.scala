...
sealed trait Saga[F[_], A](protected val step: Step[F, A]):
...
protected case class Wrap[F[_], A](step: Step[F, A]) extends Saga[F, A](step) {
...
object Saga:
  ...
  extension [F[_]: Stepper, A](saga: Saga[F, A]) def run(): F[A] = summon[Stepper[F]].run(saga.step)

  // def compensate(
  //     action: F[A],
  //     compensation: Either[Throwable, A] => F[Unit]): Saga[F, A] =
  //   Step.Next(action, compensation)

  given [F[_]]: Monad[TL[F]] with
    def pure[A](x: A): Saga[F, A]                                     = Step.Pure(x)
  ...
  //given [F[_], A]: Conversion[Step[F, A], Saga[F, A]] = a => Wrap.apply(a)
  //given [F[_], A]: Conversion[Saga[F, A], Step[F, A]] =
//    case Wrap(a) => a
